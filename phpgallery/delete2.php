...
if (isset($_GET['id'])) {
    ...
    $stmt = $pdo->prepare('SELECT * FROM peshawarimages WHERE id = ?');
    ...
    if (isset($_GET['confirm'])) {
        if ($_GET['confirm'] == 'yes') {
            ...
            unlink($image['path']);
            $stmt = $pdo->prepare('DELETE FROM peshawarimages WHERE id = ?');
            $stmt->execute([$_GET['id']]);
            // Output msg
...
<div class="content delete">
    ...
    <div class="yesno">
        <a href="delete2.php?id=<?=$image['id']?>&confirm=yes">Yes</a>
        <a href="delete2.php?id=<?=$image['id']?>&confirm=no">No</a>
    </div>
    <?php endif; ?>
</div>
