...
if (isset($_GET['id'])) {
    ...
    $stmt = $pdo->prepare('SELECT * FROM islamabadimages WHERE id = ?');
    ...
    if (isset($_GET['confirm'])) {
        if ($_GET['confirm'] == 'yes') {
            ...
            unlink($image['path']);
            $stmt = $pdo->prepare('DELETE FROM islamabadimages WHERE id = ?');
            $stmt->execute([$_GET['id']]);
            // Output msg
...
<div class="content delete">
    ...
    <div class="yesno">
        ...
        <a href="delete3.php?id=<?=$image['id']?>&confirm=no">No</a>
    </div>
    <?php endif; ?>
</div>
